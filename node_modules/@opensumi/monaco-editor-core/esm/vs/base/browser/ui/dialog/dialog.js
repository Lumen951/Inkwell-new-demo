...
class Dialog extends lifecycle_1.Disposable {
    constructor(container, message, buttons, options) {
        ...
        if (Array.isArray(buttons) && buttons.length > 0) {
            ...
            this.buttons = [nls.localize("vs/base/browser/ui/dialog/dialog",0, "OK")];
        }
        else {
            this.buttons = [];
        }
    ...
    getIconAriaLabel() {
        let typeLabel = nls.localize("vs/base/browser/ui/dialog/dialog",1, 'Info');
        switch (this.options.type) {
            case 'error':
                typeLabel = nls.localize("vs/base/browser/ui/dialog/dialog",2, 'Error');
                break;
            case 'warning':
                typeLabel = nls.localize("vs/base/browser/ui/dialog/dialog",3, 'Warning');
                break;
            case 'pending':
                typeLabel = nls.localize("vs/base/browser/ui/dialog/dialog",4, 'In Progress');
                break;
            case 'none':
            case 'info':
            case 'question':
    ...
    async show() {
        ...
        return new Promise((resolve) => {
            ...
            if (!this.options.disableCloseAction) {
                ...
                const action = this._register(new actions_1.Action('dialog.close', nls.localize("vs/base/browser/ui/dialog/dialog",5, "Close Dialog"), themables_1.ThemeIcon.asClassName(codicons_1.Codicon.dialogClose), true, async () => {
                    resolve({
                        button: this.options.cancelId || 0,
                        checkboxChecked: this.checkbox ? this.checkbox.checked : undefined
                    });
