...
const ime_1 = require("../../../base/common/ime");
...
let TextAreaHandler = class TextAreaHandler extends viewPart_1.ViewPart {
    constructor(context, viewController, visibleRangeProvider, _keybindingService, _instantiationService) {
        ...
        this.textArea.setAttribute('aria-roledescription', nls.localize("vs/editor/browser/controller/textAreaHandler",0, "editor"));
    ...
    _getWordBeforePosition(position) {
        const lineContent = this._context.viewModel.getLineContent(position.lineNumber);
        const wordSeparators = (0, wordCharacterClassifier_1.getMapForWordSeparators)(this._context.configuration.options.get(130 /* EditorOption.wordSeparators */));
    ...
    _getAriaLabel(options) {
        ...
        if (accessibilitySupport === 1 /* AccessibilitySupport.Disabled */) {
            ...
            const editorNotAccessibleMessage = nls.localize("vs/editor/browser/controller/textAreaHandler",1, "The editor is not accessible at this time.");
            if (toggleKeybindingLabel) {
                return nls.localize("vs/editor/browser/controller/textAreaHandler",2, "{0} To enable screen reader optimized mode, use {1}", editorNotAccessibleMessage, toggleKeybindingLabel);
            }
            else if (runCommandKeybindingLabel) {
                return nls.localize("vs/editor/browser/controller/textAreaHandler",3, "{0} To enable screen reader optimized mode, open the quick pick with {1} and run the command Toggle Screen Reader Accessibility Mode, which is currently not triggerable via keyboard.", editorNotAccessibleMessage, runCommandKeybindingLabel);
            }
            else if (keybindingEditorKeybindingLabel) {
                return nls.localize("vs/editor/browser/controller/textAreaHandler",4, "{0} Please assign a keybinding for the command Toggle Screen Reader Accessibility Mode by accessing the keybindings editor with {1} and run it.", editorNotAccessibleMessage, keybindingEditorKeybindingLabel);
            }
            else {
                // SOS
