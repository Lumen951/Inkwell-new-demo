...
const markdownRenderer_1 = require("../../../browser/widget/markdownRenderer/browser/markdownRenderer");
...
let MarkdownHoverParticipant = class MarkdownHoverParticipant {
    ...
    createLoadingMessage(anchor) {
        return new MarkdownHover(this, anchor.range, [new htmlContent_1.MarkdownString().appendText(nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",0, "Loading..."))], false, 2000);
    }
    computeSync(anchor, lineDecorations) {
        ...
        if (stopRenderingLineAfter >= 0 && lineLength > stopRenderingLineAfter && anchor.range.startColumn >= stopRenderingLineAfter) {
            ...
            result.push(new MarkdownHover(this, anchor.range, [{
                    value: nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",1, "Rendering paused for long line for performance reasons. This can be configured via `editor.stopRenderingLineAfter`.")
                }], false, index++));
        }
        if (!stopRenderingMessage && typeof maxTokenizationLineLength === 'number' && lineLength >= maxTokenizationLineLength) {
            result.push(new MarkdownHover(this, anchor.range, [{
                    value: nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",2, "Tokenization is skipped for long lines for performance reasons. This can be configured via `editor.maxTokenizationLineLength`.")
                }], false, index++));
        }
